...
def splitter(train_perc, valid_perc, test_perc, chosen_genres, tracks_dataframe, common_artist = False):
#INPUTS
...
#common_artist = boolean for having common artist in training, validation, and test data. Default = False
        ...
        if len(genre_tracks) != 0 and len(genre_tracks) != 3:
            ...
            if not common_artist:
                ...
                for i in range(dataset_size):
                    if (i + 1) % 2 == 0 and len(validation_tracks) < validation_size:
                        validation_tracks.append(genreframe[i: (i + 1)])
                    elif (i + 1) % 3 == 0 and len(test_tracks) < test_size:
                        test_tracks.append(genreframe[i: (i + 1)])
                    elif len(training_tracks) < training_size:
                        training_tracks.append(genreframe[i:(i+1)])
                training_tracks = pd.concat(training_tracks)
                validation_tracks = pd.concat(validation_tracks)
                test_tracks = pd.concat(test_tracks)
...
def track_extractor(chosen_genres):
#INPUT

#chosen_genres = array of chosen genres for extracting the track names
#OUTPUT

#train_tracks_list = a list of training tracks from different genres. 

#valid_tracks_list = a list of validation tracks from different genres. 

#test_tracks_list = a list of test tracks from different genres. 

#Each element of these lists is an array of track names from a specific genre
    ...
    for genre in chosen_genres:
        ...
        if is_train_path:
            train_tracks = np.array(os.listdir(train_path))
            train_tracks_list.append(train_tracks)
            print(str(len(train_tracks)) + " training tracks from genre " + genre + " extracted.")

        if is_validation_path:
            validation_tracks = np.array(os.listdir(validation_path))
            valid_tracks_list.append(validation_tracks)
            print(str(len(validation_tracks)) + " validation tracks from genre " + genre + " extracted.")

        if is_test_path:
            test_tracks = np.array(os.listdir(test_path))
            test_tracks_list.append(test_tracks)
            print(str(len(test_tracks)) + " test tracks from genre " + genre + " extracted.")
            
    return  train_tracks_list, valid_tracks_list, test_tracks_list

def corrupted_file_remover(tracks_list):
    corrupted_file_names = ["127336.mp3", "133297.mp3",  "108925.mp3"]
    for file_name in corrupted_file_names:
        for genre_num, genre_list in enumerate(tracks_list):
            if file_name in genre_list:
                tracks_list[genre_num] = np.delete(genre_list, np.where(genre_list == file_name))
